...
16. **LIFETIME POSITIONING — this is NOT a "baby's first year" product (Dan, 2026-06-30, applies to EVERY session).** Legacy Odyssey grows with the child for their **whole childhood and beyond** — the goal is to retain each customer through age 18 and ideally past it. ALL marketing, ad copy, captions, emails, blog posts, listings, and briefs must SAY or IMPLY that this is a lasting record that keeps growing year after year — NOT a one-time keepsake filled out only in the first 12 months. Avoid any framing that caps the value at infancy ("capture the first year", "baby's first months" as the whole pitch). The intro **price** is "$29 first year" (a billing fact, fine to state where allowed) — but the **product's purpose is lifelong**. NOTE the tension with rule #12: convey "lasting / grows with them their whole childhood" WITHOUT ever using the banned word "forever".
